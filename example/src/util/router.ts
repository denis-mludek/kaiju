...
export interface Route<RT extends string = string> extends CurrentStateWithParams {
...
export function startApp<RT extends string>(options: RouterOptions<RT>): void {
  ...
  const routeByName: Obj<RouteDef<RT>> = {}
  ...
  function transformRouteTree(
    ...
    return State(route.uri, {
      enter() {
        components.push(
          route.enter(router as {} as RouterAPI, currentRoute!))
      },
      update() {
        if (route.update)
